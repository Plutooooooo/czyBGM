...
def gridSearch(x_train, y_train):
    ...
    grid = GridSearchCV(AdaBoostClassifier(), param_grid, cv=10, n_jobs=-1,scoring='roc_auc',iid=False)
    # 训练模型
...
def plot_learning_curve(estimator, title, X, y,
    ...
    plt.legend(loc="best")
    plt.savefig('F:/CZY/results/all/1024/AdaBoost_learning_curve.png')
    plt.show()
    return plt
...
def plot_classification_report(cr, title=None, cmap=ddlheatmap):
    ...
    plt.ylabel('Classes')
    plt.xlabel('Measures')
    plt.savefig('F:/CZY/results/all/1024/AdaBoost_report.png')
    plt.show()

if __name__ == '__main__':
    AdaBoost(*load_data('F:/CZY/datasets/all/1024data.csv'))